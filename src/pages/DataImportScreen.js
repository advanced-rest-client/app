...
export class DataImportScreen extends ApplicationScreen {
  ...
  async readDriveFile(fileId) {
    if (!fileId) {
      this.reportCriticalError(`Unable to read a Google Drive file. The identifier is not specified`);
      return;
    }
    const id = new Date().toISOString();
    Events.Process.loadingstart(this.eventTarget, id, 'Reading file.');
    try {
      const result = await Events.Google.Drive.read(this.eventTarget, fileId);
      if (!result) {
        throw new Error('Google Drive connector did not return any data.');
      }
      this.processGoogleDriveData(result);
    } catch (cause) {
      // eslint-disable-next-line no-console
      console.error(cause);
   ...
   */
  async processGoogleDriveData(data) {
    const id = new Date().toISOString();
  ...
  async processArcData(contents) {
    ...
    try {
      ...
      this.reportCriticalError('Not an ARC file. Unable to parse a JSON content.');
      Events.Process.loadingstop(this.eventTarget, id);
      return;
    }
    if (!isArcFile(data)) {
      this.reportCriticalError('Not an ARC file. Unknown format.');
      Events.Process.loadingstop(this.eventTarget, id);
      return;
    }
    ...
    const processor = new ImportNormalize();
    try {
      const importData = await processor.normalizeArcData(data);
      this.inspectorData = importData;
      this.route = 'inspect';
    } catch (e) {
  ...
  async processPostmanData(contents) {
    ...
    try {
      ...
      this.reportCriticalError('Not a Postman file. Unable to parse a JSON content.');
      Events.Process.loadingstop(this.eventTarget, id);
      return;
    }
    if (!isPostman(data)) {
      this.reportCriticalError('Not a Postman file. Unknown format.');
      Events.Process.loadingstop(this.eventTarget, id);
      return;
    }
    const processor = new ImportNormalize();
    try {
      const importData = await processor.normalizePostman(data);
      this.inspectorData = importData;
      this.route = 'inspect';
    } catch (e) {
      this.reportCriticalError(e.message);
  ...
  pageTemplate(route) {
    ...
    <main id="main">
      ...
      ${this.driveProcessingTemplate(route)}
    </main>
    `;
  }
  ...
  importApiTemplate(route) {
    ...
    <div class="screen centered">
      ...
      <p>The imported API is used to render a documentation for the API and to pre-fill the HTTP editor with API data.</p>
      ${this.importApiAdditionalInfo()}
      <div class="main-actions">
        <anypoint-button ?anypoint="${anypoint}" @click="${this.backHandler}">Back</anypoint-button>
        <anypoint-button emphasis="high" ?anypoint="${anypoint}" @click="${this.selectApiHandler}">Select file</anypoint-button>
      </div>
    </div>
    `;
  }
  ...
  /**
   * @returns {TemplateResult|string} The template for any additional info to be rendered depending on the platform.
   */
  ...
  importArcTemplate(route) {
    ...
    <div class="screen centered">
      ...
      <anypoint-button ?anypoint="${anypoint}" @click="${this.backHandler}">Back</anypoint-button>
      <anypoint-button emphasis="high" ?anypoint="${anypoint}" @click="${this.selectArcHandler}">Select file</anypoint-button>
    </div>
    `;
  }
  ...
  importPostmanTemplate(route) {
    ...
    <div class="screen centered">
      ...
      <anypoint-button ?anypoint="${anypoint}" @click="${this.backHandler}">Back</anypoint-button>
      <anypoint-button emphasis="high" ?anypoint="${anypoint}" @click="${this.selectPostmanHandler}">Select file</anypoint-button>
    </div>
    `;
  }
  ...
  /**
   * @param {string} route The current route
   * @returns {TemplateResult|string} The template for the drive file loading info.
   */
  driveProcessingTemplate(route) {
    ...
    return html`
    <div class="screen centered">
      <h2>Importing Google Drive file</h2>
      <arc-icon class="arc-icon" icon="driveColor"></arc-icon>
      <p>Please wait while reading Google Drive file...</p>
    </div>
    `;
  }
}
