...
import { Events, ImportEvents, ArcModelEvents, EventTypes } from '@advanced-rest-client/events';
...
class ComponentDemoPage extends ExportHandlerMixin(DemoPage) {
  constructor() {
    ...
    window.addEventListener(EventTypes.Navigation.popupMenu, this.popupHandler.bind(this));
    window.addEventListener(EventTypes.Navigation.helpTopic, this.helpHandler.bind(this));
   ...
   */
  popupHandler(e) {
    const url = `popup-menu.html?type=${e.menu}`;
  ...
  proxyNavigation(data) {
    switch (data.type) {
      case 'project': Events.Navigation.navigateProject(document.body, data[0], data[1]); break;
      case 'request': Events.Navigation.navigateRequest(document.body, data[0], data[1], data[2]); break;
      case 'api': Events.Navigation.navigateRestApi(document.body, data[0], data[1], data[2]); break;
      case 'navigate': Events.Navigation.navigate(document.body, data.route, data.opts); break;
      default: 
    }
  }
